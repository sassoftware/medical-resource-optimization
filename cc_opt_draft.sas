proc optmodel;
 	...
 	set FAC_SLINE_SSERV_RESOURCES;
	set FAC_SLINE_SSERV = setof {<f,sl,ss,r> in FAC_SLINE_SSERV} <f,sl,ss>;
	set RESOURCES = setof {<f,sl,ss,r> in FAC_SLINE_SSERV} r;
	set FACILITIES = setof {<f,sl,ss,r> in FAC_SLINE_SSERV} f;
	set SERVICELINES = setof {<f,sl,ss,r> in FAC_SLINE_SSERV} sl;
	set SUBSERVICES = setof {<f,sl,ss,r> in FAC_SLINE_SSERV} ss;
	set DAYS;
...
/* 	num maxPctReschedule{FAC_SLINE_SSERV}; */
	...
	impvar TotalPatientsDay{<f,sl,ss> in FAC_SLINE_SSERV, d in DAYS}:
		sum{d1 in DAYS: max(min{d in DAYS},(d - losMean[f,sl,ss] + 1)) <= d1 <= d} NumPatientsAccept[f,sl,ss,d1];

	con Maximum_Demand{<f,sl,ss> in FAC_SLINE_SSERV, d in DAYS}:
		NumPatientsAccept[f,sl,ss,d] <= demand[f,sl,ss,d];

	con Resources_Capacity{<f,sl,ss,r> in FAC_SLINE_SSERV_RESOURCES,DAYS}:
		TotalPatientsDay[f,sl,ss] <= capacity[f,sl,ss,r];
