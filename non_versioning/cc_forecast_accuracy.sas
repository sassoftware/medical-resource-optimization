...
%macro cc_forecast_accuracy(
   ...
   proc cas;
      aggregation.aggregate / 
         table={caslib="&_worklib.", name="&input_demand"} 
   ...
   /* Get planning horizon from the parameters table (replacing sql block) */
   proc cas;
      aggregation.aggregate / 
         table={caslib="&_worklib.", name="input_opt_parameters_pp", where= "parm_name = 'PLANNING_HORIZON'" }
   ...
   %do j=1 %to &model_cnt.;
      ...
      data &outlib..&output_fa_fit_fcst. (promote=yes);
         merge &outlib..output_fd_demand_fcst (in=a keep = &hierarchy. predict_date daily_predict rename = (predict_date=date))
      ...
      proc cas;
         aggregation.aggregate /
            table={caslib="&_worklib.", name="_tmp_fcst"
            ...
            casOut={caslib="&_worklib.", name="_tmp_fa_agg", replace=true};
         run;
      quit;
      ...
      proc cas;
         aggregation.aggregate /
            table={caslib="&_worklib.", name="_tmp_fa_ape"
            ...
            casOut={caslib="&_worklib.", name="_tmp_fa_mape", replace=true};
         run;
      quit;
   ...
   proc FEDSQL sessref=mysess;
      ...
      on A.facility = B.facility AND A.service_line=B.service_line
